...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", type=str, default="../data/PeopleSnapshot", help="path to PeopleSnapshot") #required=True,
    parser.add_argument("--subject", type=str, default="female-3-casual", help="sequence to process")
    ...
    outdir = Path(f"../data/PeopleSnapshot/preprocessed_for_3DGS/{args.subject}")
    ...
    for i in tqdm.trange(frame_cnt):
        ...
        img_path = f"{image_dir}/{i:06d}.png"
        ok, frame = cap.read()
        if not ok: break
    ...
    for i, mask in enumerate(tqdm.tqdm(masks)):
        # mask_path = f"{mask_dir}/{i:06d}.npy"
        mask_path = f"{mask_dir}/{i:06d}.png"
        mask = cv2.undistort(mask, K, dist_coeffs)
        mask = mask[:,:,None]
        mask2 = np.concatenate((mask, mask, mask), axis=2)
        mask = mask2*255
