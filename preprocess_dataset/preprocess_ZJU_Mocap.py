...
from preprocess_dataset.human_body_prior.body_model.body_model import BodyModel

from preprocess_dataset.easymocap.smplmodel import load_model
...
if __name__ == '__main__':
    ...
    args.data_dir = "data/ZJUMoCap"
    args.out_dir = "data/ZJUMoCap/preprocessed_for_3DGS"
    args.seqname = "CoreView_377"
    smpl_model_dir = "body_models/smpl2"
    smpl_gender = "NEUTRAL"
    ...
    model_fn = 'SMPL_{}.{ext}'.format(smpl_gender.upper(), ext='pkl')
    smpl_model_file = os.path.join(smpl_model_dir, model_fn)
    body_model = BodyModel(bm_path=smpl_model_file, num_betas=10, batch_size=1).cuda()
    ...
    for cam_idx, cam_name in enumerate(cam_names):
        ...
        for img_file in img_files:
            ...
            if cam_idx == 0:
                ...
                body_model_em = load_model(gender='neutral', model_type='smpl', model_file=smpl_model_file)
                ...
                bone_transforms = body.bone_transforms.detach().cpu().numpy() #(1,24, 4,4)
                Jtr_posed = body.Jtr.detach().cpu().numpy() #(1, 24, 3)

                out_filename = os.path.join(smpl_out_dir, '{:06d}.npz'.format(idx))

                np.savez(out_filename,
                         minimal_shape=minimal_shape, #(6890,3)
                         betas=betas, #(10,)
                         Jtr_posed=Jtr_posed[0], #(24,3)
                         bone_transforms=bone_transforms[0], #(24,4,4)
                         trans=new_trans[0], #(3)
                         root_orient=new_root_orient[0], #(3)
                         pose_body=pose_body[0], #(63)
                         pose_hand=pose_hand[0]) #(6)
